package org.blink.blizz.countryCodePicker.ui
...
import org.blink.blizz.countryCodePicker.bottomFragment.BottomCountryPickerFragment
import org.blink.blizz.countryCodePicker.constantsCodePicker.Constants.list
import org.blink.blizz.countryCodePicker.constantsCodePicker.setCountryList
import org.blink.blizz.R
import org.blink.blizz.databinding.LayoutCodePickerBinding
import java.util.*
...
class CountryCodePicker(context: Context, attributeSet: AttributeSet) :
    ...
    init {
        ...
        setContentColor(if (color > 0) color else R.color.black)
...
//    }
    ...
    override fun onCountryItemClick(item: CountryPickerItem?) {
        item?.flagImage?.let { binding?.ivFlag?.setImageResource(it) }
        item?.phoneCode?.let { binding?.tvCode?.text = "+$it" }
        item?.codeName?.let { binding?.tvNameCode?.text = "($it)" }
        fragment?.dismiss()
